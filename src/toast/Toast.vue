...
<template>
  <div
    ...
    ref="t"
    @click="removeT"
    :class="[$style[toast?.type || 'error']]"
    class="relative rounded-md max-w-[50%] flex items-center p-4 gap-3 text-base shadow-md min-w-72 justify-between"
  >
    <p class="text-sm">{{ toast?.message }}</p>
    <button
      class="min-w-4 min-h-4 flex-shrink-0 bg-dark/20 shadow-md rounded-full text-base-clr2 right-0 top-0 w-8 h-8 flex items-center justify-center self-start"
    >
...
<style>
.leave {
  animation: leave 0.2s linear;
  animation: "leave" 0.2s linear;
}

.enter {
  animation: enter 0.2s linear;
  animation: "enter" 0.2s linear normal;
}
